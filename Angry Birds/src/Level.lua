...
function Level:init()
    ...
    -- set flag for player alien multiplying hitting spacebar
    self.multiplied = false
    self.playerCollided = false
end

function Level:spawnMultipliedAliens()

    local newAliens = {}
    for i, alien in pairs(self.launchMarker.aliens) do

        local alien1 = Alien(self.world, 'round', alien.body:getX(), alien.body:getY(), 'Player');
        local alien2 = Alien(self.world, 'round', alien.body:getX(), alien.body:getY(), 'Player');   
        
        table.insert(newAliens, alien1)
        table.insert(newAliens, alien2)

        local x, y = alien.body:getLinearVelocity()
        local angle = alien.body:getAngle()

        -- first alien should go higher, second - lower than the original
        alien1.body:setLinearVelocity(x, y * 1.5)
        alien2.body:setLinearVelocity(x, y / 2)

        -- set angle
        alien1.body:setAngle(330 * DEGREES_TO_RADIANS)
        alien2.body:setAngle(30 * DEGREES_TO_RADIANS)
    ...
    for i, alien in pairs(newAliens) do
...
function Level:update(dt)
    ...
    -- If the player alien hasn't collided with anything yet spawn 2 more on 'space' key pressed
    if not self.multiplied and love.keyboard.wasPressed('space') and not self.playerCollided then
        self.multiplied = true
        self:spawnMultipliedAliens()
    end
    ...
    if self.launchMarker.launched then
        ...
        local allPlayerAliensStopped = false
        ...
        for i, alien in pairs(self.launchMarker.aliens) do
            ...
            if xPos < 0 or (math.abs(xVel) + math.abs(yVel) < 5) then
                allPlayerAliensStopped = true
            end
        end

        if allPlayerAliensStopped then
            for i, alien in pairs(self.launchMarker.aliens) do
                alien.body:destroy()
            end
            self.launchMarker = AlienLaunchMarker(self.world)
