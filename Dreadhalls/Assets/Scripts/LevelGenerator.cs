...
public class LevelGenerator : MonoBehaviour {
	...
	[Range(0, 10)]
	public int holesInLevel = 0;
	public bool setNumberOfHolesRandomly = true;
	...
	void Start () {
		...
		if (setNumberOfHolesRandomly) {
			holesInLevel = Random.Range(0, 10);
		}

		int holesLeft = holesInLevel;
		...
		for (int z = 0; z < mazeSize; z++) {
			for (int x = 0; x < mazeSize; x++) {
				...
				} else if (holesLeft != 0 && Random.Range(0.0f, 1.0f) < 0.4f) {

					//If we still have a holes reserve, "roll a dice" to see if you shouldn't
					//generate a floor here
					holesLeft -= 1;
